...
static gchar * sunday_ansi_string_to_utf8 (const gchar * str_ansi, gint length);

guint32
_sunday_spawn_monitor_resume_thread (void * thread)
{
  return ResumeThread (thread);
}

gchar **
_sunday_spawn_monitor_get_environment (int * length)
{
  gchar ** result;
  LPWCH strings;

  strings = GetEnvironmentStringsW ();
  result = _sunday_spawn_monitor_parse_unicode_environment (strings, length);
...
gchar **
_sunday_spawn_monitor_parse_unicode_environment (void * env, int * length)
{
  GPtrArray * result;
  WCHAR * element_data;
...
gchar **
_sunday_spawn_monitor_parse_ansi_environment (void * env, int * length)
{
  GPtrArray * result;
  gchar * element_data;
  gsize element_length;
  ...
  {
    g_ptr_array_add (result, sunday_ansi_string_to_utf8 (element_data, element_length));
    element_data += element_length + 1;
  }
...
static gchar *
sunday_ansi_string_to_utf8 (const gchar * str_ansi, gint length)
{
