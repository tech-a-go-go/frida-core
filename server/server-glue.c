...
static void sunday_server_on_log_message (const gchar * log_domain, GLogLevelFlags log_level, const gchar * message, gpointer user_data);

static gboolean sunday_verbose_logging_enabled = FALSE;

void
sunday_server_environment_init (void)
{
  sunday_init_with_runtime (SUNDAY_RUNTIME_GLIB);

  g_log_set_default_handler (sunday_server_on_log_message, NULL);
}

void
sunday_server_environment_set_verbose_logging_enabled (gboolean enabled)
{
  sunday_verbose_logging_enabled = enabled;
}

void
sunday_server_environment_configure (void)
{
#if defined (HAVE_IOS) || defined (HAVE_TVOS) || defined (HAVE_XROS)
  _sunday_server_ios_tvos_configure ();
#endif
...
static void
sunday_server_on_log_message (const gchar * log_domain, GLogLevelFlags log_level, const gchar * message, gpointer user_data)
{
  if (!sunday_verbose_logging_enabled && (log_level & G_LOG_LEVEL_MASK) >= G_LOG_LEVEL_DEBUG)
    return;
