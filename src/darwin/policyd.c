...
typedef struct _SundayPolicydRequest SundayPolicydRequest;

struct _SundayPolicydRequest
{
  union __RequestUnion__sunday_policyd_subsystem body;
...
#define sunday_policyd_soften sunday_policyd_do_soften
#include "policyd-server.c"

int
sunday_policyd_main (void)
{
  kern_return_t kr;
  ...
  kr = bootstrap_check_in (bootstrap_port, SUNDAY_POLICYD_SERVICE_NAME, &listening_port);
  if (kr != KERN_SUCCESS)
    goto checkin_error;

  while (TRUE)
  {
    SundayPolicydRequest request;
    union __ReplyUnion__sunday_policyd_subsystem reply;
    ...
    handled = sunday_policyd_server (header_in, header_out);
    if (handled)
...
kern_return_t
sunday_policyd_do_soften (mach_port_t server, int pid, int * error_code)
{
