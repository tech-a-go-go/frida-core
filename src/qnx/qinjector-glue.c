...
#define SUNDAY_REMOTE_PAYLOAD_SIZE (8192)
#define SUNDAY_REMOTE_DATA_OFFSET (512)
#define SUNDAY_REMOTE_STACK_OFFSET (SUNDAY_REMOTE_PAYLOAD_SIZE - 512)
#define SUNDAY_REMOTE_DATA_FIELD(n) \
    remote_address + SUNDAY_REMOTE_DATA_OFFSET + G_STRUCT_OFFSET (SundayTrampolineData, n)

typedef struct _SundayInjectionInstance SundayInjectionInstance;
typedef struct _SundayInjectionParams SundayInjectionParams;
typedef struct _SundayCodeChunk SundayCodeChunk;
typedef struct _SundayTrampolineData SundayTrampolineData;
typedef struct _SundayFindLandingStripContext SundayFindLandingStripContext;

typedef void (* SundayEmitFunc) (const SundayInjectionParams * params, GumAddress remote_address, SundayCodeChunk * code);

struct _SundayInjectionInstance
{
  SundayQinjector * qinjector;
  guint id;
  pid_t pid;
...
struct _SundayFindLandingStripContext
{
  pid_t pid;
  GumAddress result;
};

static gboolean sunday_emit_and_remote_execute (SundayEmitFunc func, const SundayInjectionParams * params, GumAddress * result, GError ** error);

static void sunday_emit_payload_code (const SundayInjectionParams * params, GumAddress remote_address, SundayCodeChunk * code);

static GumAddress sunday_remote_alloc (pid_t pid, size_t size, int prot, GError ** error);
static int sunday_remote_dealloc (pid_t pid, GumAddress address, size_t size, GError ** error);
static int sunday_remote_pthread_create (pid_t pid, GumAddress address, GError ** error);
static int sunday_remote_msync (pid_t pid, GumAddress remote_address, gint size, gint flags, GError ** error);
static gboolean sunday_remote_write (pid_t pid, GumAddress remote_address, gconstpointer data, gsize size, GError ** error);
static gboolean sunday_remote_write_fd (gint fd, GumAddress remote_address, gconstpointer data, gsize size, GError ** error);
static gboolean sunday_remote_call (pid_t pid, GumAddress func, const GumAddress * args, gint args_length, GumAddress * retval, GError ** error);

static GumAddress sunday_resolve_remote_libc_function (int remote_pid, const gchar * function_name);

static GumAddress sunday_resolve_remote_library_function (int remote_pid, const gchar * library_name, const gchar * function_name);
static GumAddress sunday_find_library_base (pid_t pid, const gchar * library_name, gchar ** library_path);

static SundayInjectionInstance *
sunday_injection_instance_new (SundayQinjector * qinjector, guint id, pid_t pid, const char * temp_path)
{
  SundayInjectionInstance * instance;

  instance = g_slice_new0 (SundayInjectionInstance);
...
static void
sunday_injection_instance_free (SundayInjectionInstance * instance, SundayUnloadPolicy unload_policy)
{
  if (instance->remote_payload != 0 && unload_policy == SUNDAY_UNLOAD_POLICY_IMMEDIATE)
  {
    GError * error = NULL;

    sunday_remote_dealloc (instance->pid, instance->remote_payload, SUNDAY_REMOTE_PAYLOAD_SIZE, &error);
  ...
  g_slice_free (SundayInjectionInstance, instance);
}

void
_sunday_remote_thread_session_receive_pulse (void * opaque_instance, SundayQnxPulseCode * code, gint * val, GError ** error)
{
  SundayInjectionInstance * instance = opaque_instance;
  int res;
  struct _pulse pulse;
...
gboolean
_sunday_remote_thread_session_thread_is_alive (guint pid, guint tid)
{
...
void
_sunday_qinjector_free_instance (SundayQinjector * self, void * instance, SundayUnloadPolicy unload_policy)
{
  sunday_injection_instance_free (instance, unload_policy);
}

guint
_sunday_qinjector_do_inject (SundayQinjector * self, guint pid, const gchar * path, const gchar * entrypoint, const gchar * data,
    const gchar * temp_path, GError ** error)
{
  SundayInjectionInstance * instance;
  SundayInjectionParams params = { pid, path, entrypoint, data };

  instance = sunday_injection_instance_new (self, self->next_instance_id++, pid, temp_path);

  params.channel_id = instance->channel_id;
  params.remote_address = sunday_remote_alloc (pid, SUNDAY_REMOTE_PAYLOAD_SIZE, PROT_READ | PROT_WRITE | PROT_EXEC, error);
  if (params.remote_address == 0)
    goto beach;
  instance->remote_payload = params.remote_address;

  if (!sunday_emit_and_remote_execute (sunday_emit_payload_code, &params, NULL, error))
    goto beach;
...
beach:
  {
    sunday_injection_instance_free (instance, SUNDAY_UNLOAD_POLICY_IMMEDIATE);
    return 0;
  }
}

static gboolean
sunday_emit_and_remote_execute (SundayEmitFunc func, const SundayInjectionParams * params, GumAddress * result,
    GError ** error)
{
  ...
  data = (SundayTrampolineData *) (code.bytes + SUNDAY_REMOTE_DATA_OFFSET);
  ...
  if (!sunday_remote_write (params->pid, params->remote_address, code.bytes, SUNDAY_REMOTE_DATA_OFFSET + sizeof (SundayTrampolineData), error))
    return FALSE;
   ...
   */
  if (sunday_remote_msync (params->pid, params->remote_address, SUNDAY_REMOTE_PAYLOAD_SIZE, MS_INVALIDATE_ICACHE, error) != 0)
    return FALSE;

  if (sunday_remote_pthread_create (params->pid, params->remote_address, error) != 0)
    return FALSE;
...
#define EMIT_LOAD_FIELD(reg, field) \
    gum_arm_writer_put_ldr_reg_address (&cw, ARM_REG_##reg, SUNDAY_REMOTE_DATA_FIELD (field)); \
    EMIT_LDR (reg, reg)
#define EMIT_STORE_FIELD(field, reg) \
    gum_arm_writer_put_ldr_reg_address (&cw, ARM_REG_R0, SUNDAY_REMOTE_DATA_FIELD (field)); \
...
static void
sunday_emit_payload_code (const SundayInjectionParams * params, GumAddress remote_address, SundayCodeChunk * code)
{
  GumArmWriter cw;
  ...
  EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "ConnectAttach_r"),
      5,
  ...
  gum_arm_writer_put_call_address_with_arguments (&cw, sunday_resolve_remote_libc_function (params->pid, "gettid"), 0);
  EMIT_MOVE (R3, R0);

  EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "MsgSendPulse_r"),
      4,
      ARG_REG (R7),
      ARG_IMM (-1),
      ARG_IMM (SUNDAY_QNX_PULSE_CODE_HELLO),
      ARG_REG (R3));
  ...
  {
    EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "dlopen"),
        2,
  ...
  EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "dlsym"),
      2,
      ARG_REG (R6),
      ARG_IMM (SUNDAY_REMOTE_DATA_FIELD (entrypoint_name)));
  gum_arm_writer_put_mov_reg_reg (&cw, ARM_REG_R5, ARM_REG_R0);

  EMIT_LDR_U32 (R0, SUNDAY_UNLOAD_POLICY_IMMEDIATE);
  ...
  EMIT_LDR (R0, SP);
  EMIT_CMP (R0, SUNDAY_UNLOAD_POLICY_IMMEDIATE);
  EMIT_B_COND (NE, skip_dlclose);
  {
    EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "dlclose"),
        1,
        ARG_REG (R6));
  }
  ...
  EMIT_LDR (R0, SP);
  EMIT_CMP (R0, SUNDAY_UNLOAD_POLICY_DEFERRED);
  EMIT_B_COND (EQ, skip_detach);
  {
    EMIT_LOAD_FIELD (R0, worker_thread);
    EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "pthread_detach"),
        1,
        ARG_REG (R0));
  }
  ...
  EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "MsgSendPulse_r"),
      ...
      ARG_REG (R7),
      ARG_IMM (-1),
      ARG_IMM (SUNDAY_QNX_PULSE_CODE_BYE),
      ARG_REG (R3));

  EMIT_CALL_IMM (sunday_resolve_remote_libc_function (params->pid, "ConnectDetach_r"),
      1,
      ARG_REG (R7));
...
static GumAddress
sunday_remote_alloc (pid_t pid, size_t size, int prot, GError ** error)
{
  GumAddress args[] = {
    0,
    size,
  ...
  GumAddress retval = 0;
  GumAddress function = sunday_resolve_remote_libc_function (pid, "mmap");
  ...
  if (!sunday_remote_call (pid, function, args, G_N_ELEMENTS (args), &retval, error))
    return 0;
...
static int
sunday_remote_dealloc (pid_t pid, GumAddress address, size_t size, GError ** error)
{
  GumAddress args[] = {
    address,
    size
  ...
  GumAddress retval;
  GumAddress function = sunday_resolve_remote_libc_function (pid, "munmap");
...
static int
sunday_remote_pthread_create (pid_t pid, GumAddress remote_address, GError ** error)
{
  GumAddress args[] = {
    SUNDAY_REMOTE_DATA_FIELD (worker_thread),
    0,
    remote_address,
    0
  };
  GumAddress retval;
  GumAddress function = sunday_resolve_remote_libc_function (pid, "pthread_create");
...
static int
sunday_remote_msync (pid_t pid, GumAddress remote_address, gint size, gint flags, GError ** error)
{
  GumAddress args[] = {
    remote_address,
    size,
  ...
  GumAddress retval;
  GumAddress function = sunday_resolve_remote_libc_function (pid, "msync");
...
sunday_remote_write (pid_t pid, GumAddress remote_address, gconstpointer data, gsize size, GError ** error)
{
  ...
  result = sunday_remote_write_fd (fd, remote_address, data, size, error);
...
static gboolean
sunday_remote_write_fd (gint fd, GumAddress remote_address, gconstpointer data, gsize size, GError ** error)
{
  long ret;
...
static gboolean
sunday_remote_call (pid_t pid, GumAddress func, const GumAddress * args, gint args_length, GumAddress * retval, GError ** error)
{
  ...
  for (i = args_length - 1; i >= 4; i--)
  {
    ...
    if (!sunday_remote_write_fd (fd, modified_registers.arm.gpr[GUM_QNX_ARM_REG_SP], &args[i],
        4, error))
      goto beach;
  }
...
static GumAddress
sunday_resolve_remote_libc_function (int remote_pid, const gchar * function_name)
{
  return sunday_resolve_remote_library_function (remote_pid, "libc", function_name);
}

static GumAddress
sunday_resolve_remote_library_function (int remote_pid, const gchar * library_name, const gchar * function_name)
{
  ...
  local_base = sunday_find_library_base (getpid (), library_name, &local_library_path);
  g_assert (local_base != 0);

  remote_base = sunday_find_library_base (remote_pid, library_name, &remote_library_path);
  if (remote_base == 0)
  {
...
static GumAddress
sunday_find_library_base (pid_t pid, const gchar * library_name, gchar ** library_path)
{
  GumAddress result = 0;
  gchar * as_path = NULL;
  int fd = -1;
