...
GVariant *
_sunday_icon_from_process_or_file (DWORD pid, WCHAR * filename, SundayIconSize size)
{
  GVariant * icon;

  icon = _sunday_icon_from_process (pid, size);
  if (icon == NULL)
    icon = _sunday_icon_from_file (filename, size);

  return icon;
}

GVariant *
_sunday_icon_from_process (DWORD pid, SundayIconSize size)
{
  ...
  if (main_window != NULL)
  {
    ...
    else if (size == SUNDAY_ICON_LARGE)
    {
  ...
  if (icon != NULL)
    result = _sunday_icon_from_native_icon_handle (icon, size);

  return result;
}

GVariant *
_sunday_icon_from_file (WCHAR * filename, SundayIconSize size)
{
  GVariant * result = NULL;
  SHFILEINFOW shfi = { 0, };
  UINT flags;
  ...
  if (shfi.hIcon != NULL)
  {
    result = _sunday_icon_from_native_icon_handle (shfi.hIcon, size);
...
GVariant *
_sunday_icon_from_resource_url (WCHAR * resource_url, SundayIconSize size)
{
  ...
  *p = L'\0';
  ...
  ret = ExtractIconExW (resource_file, resource_id, (size == SUNDAY_ICON_LARGE) ? &icon : NULL, (size == SUNDAY_ICON_SMALL) ? &icon : NULL, 1);
...
GVariant *
_sunday_icon_from_native_icon_handle (HICON icon, SundayIconSize size)
{
  GVariant * result;
  HDC dc;
