...
static gboolean sunday_add_process_metadata (GHashTable * parameters, guint pid, HANDLE process, SundayEnumerateProcessesOperation * op);

static gboolean sunday_get_process_filename (HANDLE process, WCHAR * name, DWORD name_capacity);
static GVariant * sunday_get_process_user (HANDLE process);
static GVariant * sunday_get_process_start_time (HANDLE process);

static GHashTable * sunday_build_ppid_table (void);
static guint sunday_get_frontmost_pid (void);

static GDateTime * sunday_parse_filetime (const FILETIME * ft);
static gint64 sunday_filetime_to_unix (const FILETIME * ft);
...
sunday_system_enumerate_processes (SundayProcessQueryOptions * options, int * result_length)
{
  ...
  op.ppid_by_pid = NULL;
  op.frontmost_pid = (op.scope != SUNDAY_SCOPE_MINIMAL) ? sunday_get_frontmost_pid () : 0;
  ...
  else
  {
    ...
    for (i = 0; i != bytes_returned / sizeof (DWORD); i++)
      sunday_collect_process_info (pids[i], &op);
...
sunday_collect_process_info (guint pid, SundayEnumerateProcessesOperation * op)
{
  ...
  if (!sunday_get_process_filename (handle, program_path_utf16, G_N_ELEMENTS (program_path_utf16)))
    goto beach;
  ...
  if (op->scope != SUNDAY_SCOPE_MINIMAL)
  {
    ...
    still_alive = sunday_add_process_metadata (info.parameters, pid, handle, op);
  ...
  if (op->scope == SUNDAY_SCOPE_FULL)
  {
    ...
    small_icon = _sunday_icon_from_process_or_file (pid, program_path_utf16, SUNDAY_ICON_SMALL);
    if (small_icon != NULL)
    {
    ...
    large_icon = _sunday_icon_from_process_or_file (pid, program_path_utf16, SUNDAY_ICON_LARGE);
    if (large_icon != NULL)
    {
...
static gboolean
sunday_add_process_metadata (GHashTable * parameters, guint pid, HANDLE process, SundayEnumerateProcessesOperation * op)
{
  GVariant * user;
  guint ppid;
  GVariant * started;

  user = sunday_get_process_user (process);
  if (user == NULL)
    return FALSE;
  g_hash_table_insert (parameters, g_strdup ("user"), g_variant_ref_sink (user));

  if (op->ppid_by_pid == NULL)
    op->ppid_by_pid = sunday_build_ppid_table ();
  ...
  started = sunday_get_process_start_time (process);
  if (started == NULL)
    return FALSE;
...
static gboolean
sunday_get_process_filename (HANDLE process, WCHAR * name, DWORD name_capacity)
{
  gsize name_length;
...
static GVariant *
sunday_get_process_user (HANDLE process)
{
...
static GVariant *
sunday_get_process_start_time (HANDLE process)
{
  GVariant * result;
  ...
  creation_dt = sunday_parse_filetime (&creation_time);
...
static GDateTime *
sunday_parse_filetime (const FILETIME * ft)
{
  GDateTime * result;
  gint64 unix_time, unix_sec, unix_usec;
  GDateTime * dt;

  unix_time = sunday_filetime_to_unix (ft);
...
static gint64
sunday_filetime_to_unix (const FILETIME * ft)
{
  ULARGE_INTEGER u;
