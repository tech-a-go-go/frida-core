...
static WCHAR * sunday_command_line_from_argv (gchar ** argv, gint argv_length);
static WCHAR * sunday_environment_block_from_envp (gchar ** envp, gint envp_length);
    ...
    gboolean suspended, void ** handle, guint * id, GError ** error)
{
  ...
  command_line = sunday_command_line_from_argv (argv, argv_length);
  environment = sunday_environment_block_from_envp (envp, envp_length);
...
int
sunday_test_process_backend_join (void * handle, guint timeout_msec, GError ** error)
{
  DWORD exit_code;
...
static WCHAR *
sunday_command_line_from_argv (gchar ** argv, gint argv_length)
{
  GString * line;
  WCHAR * line_utf16;
  gint i;
...
static WCHAR *
sunday_environment_block_from_envp (gchar ** envp, gint envp_length)
{
  GString * block;
  gint i;
